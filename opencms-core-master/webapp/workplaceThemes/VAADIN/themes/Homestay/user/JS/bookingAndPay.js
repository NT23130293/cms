 ...
 *        -> popup thanh toán (QR / ví / thẻ) -> màn hình thành công.
...
const PAGES = { home: 'homepage.html', detail: 'homestay-detail.html', myBookings: 'my-bookings.html' };
...
function initStaticFields() {
    ...
    $('okMyBookings').href = PAGES.myBookings;
    $('okHome').href = PAGES.home;
...
function bindEvents() {
    ...
    $('okCopyBtn').addEventListener('click', () => copyText(state.bookingCode, 'Đã sao chép mã đặt phòng!'));
...
function finishBooking() {
    ...
    } catch (e) { /* trình duyệt chặn lưu trữ: bỏ qua, vẫn hiện màn hình thành công */ }

    state.processing = false;
    $('payOverlay').classList.remove('active');
    document.body.style.overflow = '';
    showSuccess(record, p);
}

function showSuccess(record, p) {
    $('bkBody').hidden = true;
    $('bkSuccess').hidden = false;
    $('mobileBar').style.display = 'none';
    $('bkBackLink').hidden = true;
    $('bkSteps').closest('.bk-top-row').querySelector('.bk-title').textContent = 'Hoàn tất đặt phòng';
    setStep(3);

    $('okCode').textContent = record.code;
    $('okSub').textContent = `Cảm ơn ${record.contact.fullName}! Chúng tôi đã gửi email xác nhận tới ${record.contact.email}.`;
    $('okHomestay').textContent = record.homestay;
    $('okRoom').textContent = record.room.name;
    $('okCheckin').textContent = fmtDate(record.checkin) + ' · từ 14:00';
    $('okCheckout').textContent = fmtDate(record.checkout) + ' · trước 12:00';
    $('okGuests').textContent = `${record.guests} khách · ${record.nights} đêm`;
    $('okContact').textContent = `${record.contact.fullName} · ${record.contact.phone}`;
    $('okPaid').textContent = `${fmtVND(p.payNow)} (${METHOD_LABELS[record.method]})`;
    $('okRemainingBox').hidden = p.payLater <= 0;
    $('okRemaining').textContent = fmtVND(p.payLater);

    const next = [
        'Chủ nhà sẽ nhắn mã mở khóa cho bạn trước ngày nhận phòng 1 ngày. Bạn tự nhận phòng trong khung 14:00 - 21:00.',
        `Email xác nhận kèm mã đặt phòng <b>${record.code}</b> đã được gửi tới <b>${record.contact.email}</b>.`
    ];
    if (p.payLater > 0) next.push(`Bạn thanh toán nốt <b>${fmtVND(p.payLater)}</b> khi nhận phòng.`);
    if (record.experiences.length) {
        const cost = p.experienceTotal > 0 ? `, dự kiến ${fmtVND(p.experienceTotal)}, trả trực tiếp tại homestay` : '';
        next.push(`Chủ nhà sẽ liên hệ xác nhận ${record.experiences.length} trải nghiệm bạn đã chọn${cost}.`);
    }
    next.push('Cần đổi lịch hoặc hủy phòng? Miễn phí hủy trong 48 giờ sau khi đặt.');
    $('okNextList').innerHTML = next.map((t) => `<li><i class="bi bi-check2-circle"></i><span>${t}</span></li>`).join('');
